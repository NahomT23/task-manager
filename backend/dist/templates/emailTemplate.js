...
exports.taskAssignedTemplate = exports.taskCompletedTemplate = void 0;
const taskCompletedTemplate = (task, userName, frontendUrl) => {
    const userNamesList = Array.isArray(userName) ? userName : [userName];
    return `
    <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ddd;">
      <h2 style="color: #2c3e50;">Task Completed</h2>
      <p>Task "${task.title}" has been completed by ${userNamesList.map((name) => `<li>${name}</li>`).join('')}</p>
...
exports.taskCompletedTemplate = taskCompletedTemplate;
const taskAssignedTemplate = (task, userName, frontendUrl) => {
    return `
      <div style="font-family: Arial, sans-serif; padding: 20px; border: 1px solid #ddd;">
